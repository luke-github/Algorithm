...
void left_algo(shared_ptr<BTN<int>>& node, bool is_boundary){
	if(node){
		if(is_boundary || (!node->left&&!node->right)){
			cout<<node->data<<" ";
		}
		left_algo(node->left,is_boundary);
		left_algo(node->right,is_boundary&&!node->left);
	}
}

void right_algo(shared_ptr<BTN<int>>& node, bool is_boundary){
	if(node){
		right_algo(node->left,is_boundary&&!node->right);
		right_algo(node->right,is_boundary);
		if(is_boundary || (!node->left&&!node->right)){
			cout<<node->data<<" ";
		}
	}
...
void print_exterior(shared_ptr<BTN<int>>& root){
	if(root){
		cout<<root->data<<" ";
		left_algo(root->left,true);
		right_algo(root->right,true);
	}
}
